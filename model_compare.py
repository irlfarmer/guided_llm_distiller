    ...
    def generate_response(self, question: str, max_length: int = 512) -> str:
        ...
        try:
            ...
            outputs = self.model.generate(
                ...
                max_length=max_length,
                min_length=32,  # Add minimum length
...
@st.cache_resource
def load_single_model(model_choice: str) -> Optional[ModelWrapper]:
    """Load a single model with caching"""
    try:
        if model_choice == "Teacher (OPT-2.7B)":
            return ModelWrapper("facebook/opt-2.7b")
        elif model_choice == "Student (OPT-350M)":
            return ModelWrapper("facebook/opt-350m")
        elif model_choice == "Distilled Model":
            return ModelWrapper("./final_distilled_model")
        return None
    except Exception as e:
...
def main():
    ...
    st.write("Compare responses from different models")
    ...
    model_choice = st.radio(
        ...
        ["Teacher (OPT-2.7B)", "Student (OPT-350M)", "Distilled Model"],
